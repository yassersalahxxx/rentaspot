...
const BASE_URL = "https://rent-spot-backend-f6cretc2bccehuas.francecentral-01.azurewebsites.net/"
...
export const createParking = async ({ body, handleCreateParkingSuccess, handleCreateParkingFailure }) => {
  try {
    console.log('API: Sending request with body:', JSON.stringify(body, null, 2));
    
    // Keep coordinates as strings - backend expects string format
    ...
    // Pass the complete error response data
    if (error.response?.data) {
      handleCreateParkingFailure(error.response.data);
    } else {
...
export const updateParking = async ({ id, body, handleUpdateParkingSuccess, handleUpdateParkingFailure }) => {
  try {
    console.log('API: Updating parking with body:', JSON.stringify(body, null, 2));
    
    // Keep coordinates as strings - backend expects string format
    const result = await axios.put(`${BASE_URL}parking/${id}`, { ...body })
    console.log('API: Update success response:', JSON.stringify(result.data, null, 2));
    ...
    console.error('API: Update error response:', error.response);
    console.error('API: Update error response data:', error.response?.data);
    
    // Pass the complete error response data
    if (error.response?.data) {
      handleUpdateParkingFailure(error.response.data);
    } else {
